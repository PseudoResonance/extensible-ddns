...
async def main():
    ...
    for source in config["sources"]:
        try:
            ...
            module = importlib.import_module(f"sources.{conf["type"]}")
            ipRecord[source] = await module.fetch_ip(conf, verbose=args.verbose)
        except Exception:
            print(f"Error while fetching IPs for {source}:")
            print(traceback.format_exc())
            ipRecord[source] = SourceResult(False, [])
    ...
    for transform in config["transforms"]:
        try:
            ...
            module = importlib.import_module(f"transforms.{conf["type"]}")
            ipRecord[transform] = await module.transform_ips(conf, ipRecord, verbose=args.verbose)
        except Exception:
            print(f"Error while transforming IPs with {transform}:")
            print(traceback.format_exc())
            ipRecord[transform] = SourceResult(False, [])
