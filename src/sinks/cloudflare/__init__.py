...
MANAGEMENT_COMMENT = "Managed by DDNS"
...
async def update_ips(config, ipRecord, dryrun=False, verbose=False):
    ...
    for zone in config["zones"]:
        if zone["name"] not in zoneMapping:
            print("Cloudflare: Zone " + zone["name"] + " does not exist.")
            continue
        ...
        state = []
        for record in client.dns.records.list(zone_id=zone_id, comment=MANAGEMENT_COMMENT):
            state.append(util.records.Record(record.content, record.type,
                         record.name.removesuffix("." + zone["name"]), record.ttl, frozendict({"proxied": record.proxied}), frozendict({"id": record.id})))
        if verbose:
        ...
        for recordSet in to_update:
            ...
            if dryrun:
                print(f"Cloudflare: Updating dns_record_id:{recordSet.current.internal["id"]} zone_id:{zone_id} comment:{MANAGEMENT_COMMENT} name:{record.domain} type:{
                ...
                try:
                    client.dns.records.update(
                        zone_id=zone_id, dns_record_id=recordSet.current.internal["id"], type=record.type, comment=MANAGEMENT_COMMENT, name=record.domain, content=record.content, ttl=record.ttl, proxied=record.extra["proxied"])
                    ...
                    print(f"Cloudflare: Error while updating record dns_record_id:{recordSet.current.internal["id"]} zone_id:{zone_id} comment:{MANAGEMENT_COMMENT} name:{record.domain} type:{
                        record.type} content:{record.content} ttl:{record.ttl} proxied:{record.extra["proxied"]}")
                    print(e.status_code)
                    print(e.response)
        for record in to_delete:
            if dryrun:
                print(f"Cloudflare: Deleting dns_record_id:{record.internal["id"]} zone_id:{zone_id} comment:{MANAGEMENT_COMMENT} name:{record.domain} type:{
                      record.type} content:{record.content} ttl:{record.ttl} proxied:{record.extra["proxied"]}")
                state.remove(record)
            else:
                try:
                    ...
                    print(f"Cloudflare: Error while deleting record dns_record_id:{recordSet.current.internal["id"]} zone_id:{zone_id} comment:{MANAGEMENT_COMMENT} name:{record.domain} type:{
                        record.type} content:{record.content} ttl:{record.ttl} proxied:{record.extra["proxied"]}")
                    print(e.status_code)
                    print(e.response)
        for record in to_add:
            if dryrun:
                print(f"Cloudflare: Adding zone_id:{zone_id} comment:{MANAGEMENT_COMMENT} name:{record.domain} type:{
                      record.type} content:{record.content} ttl:{record.ttl} proxied:{record.extra["proxied"]}")
                state.append(util.records.Record(record.content, record.type,
                ...
                try:
                    newRecord = client.dns.records.create(
                        zone_id=zone_id, comment=MANAGEMENT_COMMENT, name=record.domain, type=record.type, content=record.content, ttl=record.ttl, proxied=record.extra["proxied"])
                    ...
                    print(f"Cloudflare: Error while adding record zone_id:{zone_id} comment:{MANAGEMENT_COMMENT} name:{record.domain} type:{
                        record.type} content:{record.content} ttl:{record.ttl} proxied:{record.extra["proxied"]}")
                    print(e.status_code)
                    print(e.response)
        if verbose:
